...
@dataclass
class GeoSplatPriorTrainer(BaseTrainer):

    cov3d_lr: float = 1e-3
    geometry_lr: float = 1e-4
    ...
    ks_grad_reg_begin: float = 0.01
    ks_grad_reg_end: float = 0.01
    ...
    def setup(
        ...
        if model.smooth_type == 'jitter':
            ...
            optim_dict['kdks'] = Optimizer(
                category=torch.optim.Adam,
                modules=model.as_module(field_name='kdks'),
                lr=self.appearance_lr * 3,
                eps=self.base_eps,
                lr_decay=self.base_decay,
            )
            optim_dict['zs'] = Optimizer(
                category=torch.optim.Adam,
                modules=model.as_module(field_name='zs'),
                lr=self.cov3d_lr,
                eps=self.base_eps,
                lr_decay=self.base_decay,
            )
    ...
    def after_update(self, model: GeoSplatterPrior, optimizers: ModuleOptimizers, *, curr_step: int) -> None:
        if curr_step % 25 == 0:
            gc.collect()
